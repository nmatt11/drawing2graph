...
for k = 1:m
    ...
    display('Starting off with a greedy tour')
	p = greedy(s(k),D);
    display('Done with the greedy tour, can it be improved by doing swaps?')
    % Improve tour by 2-opt heuristics
	[p,L] = exchange2(p,D,K);
    display('Done doing swaps')
    % Keep best tour
	if L < Lmin
        Lmin = L;
        pmin = p;
